...
# ---- config H-OPTIMUS ----
HOPT_MANIFEST_20X = Path("/mnt/e/internship/output_LF/manifest_slides_LF.csv")
HOPT_ENCODEUR = "h_optimus_1"
HOPT_DIM = 1536

# ---- config VIRCHOW 2 ----
VIR_MANIFEST_20X = Path("/mnt/e/internship/output_LF_virchow2/manifest_slides_LF.csv")
VIR_MANIFEST_5X  = Path("/mnt/e/internship/output_LF_virchow2_5x/manifest_slides_5x.csv")
...
def rapport(nom, fichier, vrais, predits, scores, noms, types, diags):
    ...
    acc = accuracy_score(vrais, predits); auc = roc_auc_score(vrais, scores)
    print(f"\n===== {nom} =====")
...
def main():
    hopt = pd.read_csv(HOPT_MANIFEST_20X).set_index("slide")
    v20  = pd.read_csv(VIR_MANIFEST_20X).set_index("slide")
    v5   = pd.read_csv(VIR_MANIFEST_5X).set_index("slide")
